...
class RandomCreditContractGen:
    ...
    def random_contract_ts(self, *args):
        ...
        _pd = self.rng.uniform(high = 0.1, size = _end_time)
        _dates = np.arange(_end_time)
        _sector = self.rng.randint(1,4)
        return {'id' : _id, 'pd' : _pd, 'exposure' : _exposure, 'sector' : _sector, 'date' : _dates}
...
class CreditPortfolioGen:
    ...
    def generate_df(self, seed, size):
        ...
        list_of_contracts = self.pool.map(RandomCreditGen.random_contract, range(size))
